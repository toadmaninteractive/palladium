...
@Component({
    selector: 'm-d3-dashboard-plot',
    templateUrl: './d3-dashboard-plot.component.html',
    styleUrls: ['./d3-dashboard-plot.component.scss'],
})

export class D3DashboardPlotComponent extends D3Basic implements OnDestroy, OnInit, AfterViewInit {
    ...
    constructor(
        ...
        // Redraw chart on window resize toggle
        this.resize
        .pipe(
            debounceTime(400)
        )
        .subscribe(() => {
            const newWidth = this.chartContainer.nativeElement.getBoundingClientRect().width - this.padding.left - this.padding.right,
            newHeight = this.chartContainer.nativeElement.getBoundingClientRect().height - this.padding.top - this.padding.bottom;

            if (this.plotProps.width !== newWidth || this.plotProps.height !== newHeight) {
                d3.select(this.barsContainer.nativeElement).selectAll('g').remove();
                this.drawChart(this.drawingSeries, this.pristineOptions);
                setTimeout(() => this.cdr.detectChanges());
            }
        });

        // Redraw chart on menu toggle
        this.menuAsideService.menuToggled$.subscribe(() => {
            this.drawChart(this.drawingSeries, this.pristineOptions);
            setTimeout(() => this.cdr.detectChanges());
        });
    }
    ...
    drawChart(series: DataProtocol.NumericSeries[], options: VisualizationProtocol.VisualizationCommonPlot | VisualizationProtocol.VisualizationSynchronizedPlot): void {
        ...
        //     ? this.getMaxWidth(this.xPrimaryValues, options.xPrimaryValueType) * Math.cos(0.78) // 45°
        //     : 0;

        const width = this.chartContainer.nativeElement.getBoundingClientRect().width - this.padding.left - this.padding.right,
            height = this.chartContainer.nativeElement.getBoundingClientRect().height - this.padding.top - this.padding.bottom - 10;
    ...
    commonDraw(animate: boolean, series: DataProtocol.NumericSeries[]): void {
        ...
        // CHECK IF ANY TWO DATASETS OF SERIES IS EQUAL, DASH IF TRUE
        // this.dashedSeries = this.getDashedSeries(this.drawingSeries, this.legendSeriesMap);
        // this.dashedSeries = false;
    ...
    needRotateAxisLabel(): boolean {
        ...
        if (this.options$.getValue().xAxisType === VisualizationProtocol.AxisValueType.Text) {
            this.paddingXTypeLine = this.xPrimaryScale.bandwidth() / 2;
        }
        ...
        const maxPossibleWidth = (this.chartContainer.nativeElement.getBoundingClientRect().width - this.padding.left - this.padding.right) / this.calculateTickCount(labels),
            maxLabelWidth = Math.max(...labels.map(item => this.context.measureText(item).width)) + 20;
    ...
    showTooltipLine(): void {
        if (this.existLineSeries) {
            ...
            d3.select(this.chartContainer.nativeElement).selectAll<SVGGElement, any>('.dots-bg')
                .selectAll('circle')
                // @ts-ignore
                .attr<NumericPoint>('fill-opacity', 0);

            d3.select(this.chartContainer.nativeElement).select('.dots-bg.' + needleSeries.key)
                .selectAll('circle')
                // @ts-ignore
    ...
    private getTooltipContent(hoveredXLabel: string): string {
        ...
        this.drawingSeries.forEach((seriesData) => {
            ...
            result += '<li>' +
                ...
                this.getSeriesOptions(seriesData.key).name + ': ' + '<b>' + (value < 10 ? Math.round(value * 10) / 10 : Math.round(value)) + '</b></li>';
        });
        return result;
    }
    ...
    getColorByKey(key: string): string {
        if (this.getSeriesOptions(key).color) return this.getSeriesOptions(key).color;
    ...
    getMaxWidth(labels: Array<any>, labelsType: VisualizationProtocol.AxisValueType) {
        ...
        const axisTickSize = 5,
            axisPadding = 5;
        if (labelsType === VisualizationProtocol.AxisValueType.Date) labels = labels.map(item => new Date(item).toLocaleDateString('ru-RU'));
